...
describe('VerificationRequest', function () {
    test('can be instantiated with all required parameters', function () {
        ...
        expect($request->challengeHex)->toBe('test_wallet_auth_challenge_hex');
        expect($request->expectedSignerStakeAddress)->toBe('test_stake_key_address');
    ...
    test('can be created from array with fromArray method', function () {
        $data = [
            ...
            'challengeHex' => 'test_wallet_auth_challenge_hex',
            'expectedSignerStakeAddress' => 'test_stake_key_address',
            'networkMode' => 0
        ];
    ...
    test('fromArray works with real demo data', function () {
        ...
        expect($request->challengeHex)->toBe($data['challengeHex']);
        expect($request->expectedSignerStakeAddress)->toBe($data['expectedSignerStakeAddress']);
